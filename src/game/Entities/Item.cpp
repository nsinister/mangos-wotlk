...
void Item::DeleteFromDB()
{
    ...
    Item::DeleteFakeFromDB(GetGUIDLow()); // transmogrification
...
{
    GetOwner()->SetUInt32Value(PLAYER_VISIBLE_ITEM_1_ENTRYID + (GetSlot() * 2), entry);
    sObjectMgr._itemFakeEntryStore[GetGUIDLow()] = entry;
    CharacterDatabase.PExecute("REPLACE INTO custom_transmogrification (GUID, FakeEntry, Owner) VALUES (%u, %u, %u)", GetGUIDLow(), entry, GetOwner()->GetGUIDLow());
}
