...
/* ContentData
transmog                 50%    transmogrification gossip menu
...
#define TRANSMOG_SLOT_HIDDEN_ENTRYID    0
...
const char * GetSlotName(uint8 slot, WorldSession* session)
{
    switch(slot)
    {
        ...
        case EQUIPMENT_SLOT_WRISTS    : return session->GetMangosString(LANG_SLOT_NAME_WRISTS);
...
bool GossipSelect_transmog(Player* player, Creature* creature, uint32 sender, uint32 uiAction)
{
    ...
    player->GetPlayerMenu()->ClearMenus();
        ...
        case EQUIPMENT_SLOT_END: // Show items you can use
        {
            if (Item* oldItem = player->GetItemByPos(INVENTORY_SLOT_BAG_0, uiAction))
            {
                ...
                for (uint8 i = INVENTORY_SLOT_ITEM_START; i < INVENTORY_SLOT_ITEM_END; i++)
                {
                        ...
                        if (Item* newItem = player->GetItemByPos(INVENTORY_SLOT_BAG_0, i))
                        {
                            ...
                            if (player->SuitableForTransmogrification(oldItem, newItem) == ERR_FAKE_OK)
                            {
                                if (_items[lowGUID].find(display) == _items[lowGUID].end())
                                {
                                    ...
                                    player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_INTERACT_1, newItem->GetProto()->Name1, uiAction, display, session->GetMangosString(LANG_POPUP_TRANSMOGRIFY)+GetItemName(newItem, session), 0, false);
							   }
                            }
                        }
                    }
                                ...
                                if (Item* newItem = player->GetItemByPos(i, j))
                                {
                                    ...
                                    if (player->SuitableForTransmogrification(oldItem, newItem) == ERR_FAKE_OK)
                                    {
                                        if (_items[lowGUID].find(display) == _items[lowGUID].end())
                                        {
                                            limit++;
                                            _items[lowGUID][display] = newItem;
                                            player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_INTERACT_1, newItem->GetProto()->Name1, uiAction, display, session->GetMangosString(LANG_POPUP_TRANSMOGRIFY)+GetItemName(newItem, session), GOLD_COST_FUNCTION, false);}
                                    }
                                }
                            }
                        }
                    ...
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_INTERACT_1, session->GetMangosString(LANG_OPTION_HIDE_SLOT), EQUIPMENT_SLOT_END+4, uiAction);
        ...
        case EQUIPMENT_SLOT_END+2: // Remove Transmogrifications
            {
                ...
                if (removed)
                {
                    session->SendAreaTriggerMessage(session->GetMangosString(LANG_REM_TRANSMOGRIFICATIONS_ITEMS));
                    player->PlayDirectSound(3337);
                }
                else
                    session->SendNotification(session->GetMangosString(LANG_ERR_NO_TRANSMOGRIFICATIONS));
                GossipHello_transmog(player, creature);
            } break;
        ...
        case EQUIPMENT_SLOT_END+4: // Hide slot
            {
                if (Item* newItem = player->GetItemByPos(INVENTORY_SLOT_BAG_0, uiAction))
                {
                    newItem->SetFakeEntry(TRANSMOG_SLOT_HIDDEN_ENTRYID);
                    session->SendNotification(session->GetMangosString(LANG_TRANSMOG_SLOT_HIDDEN_SUCCESS));
                }
                GossipSelect_transmog(player, creature, EQUIPMENT_SLOT_END, uiAction);
            } break;
                ...
                if (Item* oldItem = player->GetItemByPos(INVENTORY_SLOT_BAG_0, sender))
                {
                    if (_items[lowGUID].find(uiAction) != _items[lowGUID].end() && _items[lowGUID][uiAction]->IsInWorld())
                    {
                        ...
                        if (newItem->GetOwnerGuid() == player->GetGUIDLow() && (newItem->IsInBag() || newItem->GetBagSlot() == INVENTORY_SLOT_BAG_0) && player->SuitableForTransmogrification(oldItem, newItem) == ERR_FAKE_OK)
                        {
                            ...
                            session->SendNotification(session->GetMangosString(LANG_ERR_NO_ITEM_SUITABLE));
                    }
                    else
                        session->SendNotification(session->GetMangosString(LANG_ERR_NO_ITEM_EXISTS));
                }
                else
                    session->SendNotification(session->GetMangosString(LANG_ERR_EQUIP_SLOT_EMPTY));
                _items[lowGUID].clear();
